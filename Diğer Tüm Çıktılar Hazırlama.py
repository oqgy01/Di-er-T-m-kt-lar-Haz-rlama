...
print(" ")
print(Fore.BLUE + "2500 TL Üzeri Siparişler - Gönderilmesi Gereken Siparişler - Faturasız Siparişler - Moda Yakamoz - Janes Toptan")
...
moda_yakamoz = input("Moda Yakamoz Hazırlansın mı? (E/H): ").strip().upper()
...
#GÖNDERİLMESİ GEREKEN SİPARİŞLER
...
if moda_yakamoz == "E":
    ...
    async def main():
        base_url = "https://www.modaymis.com/FaprikaOrderXls/BQFNLL/"
        urls = [f"{base_url}{i}/" for i in range(1, 2)]
    ...
    # Kopya dosya adı (istediğiniz adı ve konumu belirtin)
    kopya_excel = "Hazırlanan Sipariş Numaraları4.xlsx"
    ...
    old_file_path = "birlesik_excel.xlsx"
    new_file_path = "Moda Yakamoz.xlsx"
    ...
    # Excel dosyasını oku
    excel_file_path = "Moda Yakamoz.xlsx"  # Moda Yakamoz Excel dosyasının adını ve yolunu belirtin
    df = pd.read_excel(excel_file_path)
    ...
    sonuc_df = pd.read_excel("Moda Yakamoz.xlsx")

    # "Moda Yakamoz.xlsx" dosyasını güncelleme
    sonuc_df["UrunAdi Kopya"] = sonuc_df["UrunAdi"]  # "UrunAdi" sütununu kopyala
    sonuc_df["UrunAdi Kopya"] = sonuc_df["UrunAdi Kopya"].str.split("-", n=1).str[1]  # "-" den öncesini temizle

    # "UrunAdi Kopya" sütununu "Moda Yakamoz.xlsx" dosyasına ekleyerek güncelleme
    with pd.ExcelWriter("Moda Yakamoz.xlsx") as writer:
        sonuc_df.to_excel(writer, index=False)
    ...
    # "Moda Yakamoz.xlsx" dosyasını güncelleme
    sonuc_df.to_excel("Moda Yakamoz.xlsx", index=False)
    ...
    # Yeni bir Excel sayfası oluşturma ve "Id" değerlerini yazma
    with pd.ExcelWriter("Moda Yakamoz.xlsx", engine="openpyxl", mode="a") as writer:
        unique_ids.to_excel(writer, sheet_name="Unique Ids", index=False)
    ...
    def create_bat_files(data, output_folder, batch_size=28):
        ...
        while len(remaining_data) > 0:
            ...
            bat_file_path = os.path.join(output_folder, f"BAT{batch_count}.bat")
            with open(bat_file_path, "w") as file:
                link = f'start "" https://www.modaymis.com/admin/order/edit/{current_batch[0]}\n'
                file.write(link)
                file.write('timeout -t 1\n')  # Add the timeout line
    ...
    # Excel dosyasını klasöre taşı
    shutil.copy(file_path, os.path.join(output_folder, "Moda Yakamoz.xlsx"))
    ...
    # Klasör adları
    folders = ["Moda Yakamoz"]

    # Bugünkü tarihi al
    current_date = datetime.datetime.now().strftime("%Y-%m-%d")

    # Oluşturulacak zip dosyasının adı
    zip_filename = "Moda Yakamoz.zip"
    ...
    print(Fore.YELLOW + "Moda Yakamoz Siparişleri Hazırlandı")
else:
    pass
